...
from constants import (BETA0_SEARCH, DEFAULT_SEED, DELTA_BETA, DELTA_K, DELTA_RHO,
                       FIGURES_DIR, I0_SWEDEN, K0_SEARCH, N_RUNS_SEARCH, N_SWEDEN,
                       RESULTS_DIR, RHO0_SEARCH, VACC_SWEDEN, WEEKS)
...
def evaluate(k, beta, rho, graph_cache, rmse_cache, base_seed):
    ...
    summary = run_many(adjacency, beta, rho, N_INIT_SWEDEN, WEEKS, N_RUNS_SEARCH,
                       base_seed=base_seed, vacc_schedule=VACC_SWEDEN)
...
def neighbourhood(center, deltas):
    ...
    beta_values = sorted({min(0.95, max(0.05, round(bc + s * db, 4))) for s in (-1, 0, 1)})
    ...
    while True:
        ...
        while True:
            ...
            for point in neighbourhood(center, deltas):
                ...
                log_rows.append({
                    "iteration": iteration, "k": point[0],
                    "beta": format_float(point[1]), "rho": format_float(point[2]),
                    "rmse": format_float(rmse),
                })
                if rmse < best_rmse:
                    best_rmse = rmse
                    best_point = point
            print(f"[4] iter {iteration:>2} (deltas={tuple(deltas)}): "
...
def main():
    ...
    print(f"[4] BEST: k={best_k}, beta={best_beta}, rho={best_rho}, "
    ...
    summary = run_many(adjacency, best_beta, best_rho, N_INIT_SWEDEN, WEEKS, final_runs,
                       base_seed=DEFAULT_SEED + 4200, vacc_schedule=VACC_SWEDEN)
    weeks = summary["weeks"]
    final_rmse = rmse_to_real(summary["newly_infected_mean"])

    write_single_row_csv(
        RESULTS_DIR / "problem4_best_params.csv",
        {"n": N_SWEDEN, "best_k": best_k, "best_beta": format_float(best_beta),
         "best_rho": format_float(best_rho),
         "n_runs_search": N_RUNS_SEARCH, "search_rmse": format_float(best_rmse),
         "n_runs_final": final_runs, "final_rmse": format_float(final_rmse),
         "n_init_infected": N_INIT_SWEDEN, "seed": DEFAULT_SEED + 4200},
    )
    write_weekly_csv(
        RESULTS_DIR / "problem4_fit_vs_real.csv", weeks,
        {"model_newly_infected_mean": summary["newly_infected_mean"],
         "real_newly_infected": I0},
    )
    write_weekly_csv(
        RESULTS_DIR / "problem4_sirv_totals.csv", weeks,
    ...
    plot_lines(
        ...
        {"model (best params)": summary["newly_infected_mean"],
         "real data $I_0(t)$": I0},
        "week", "average number of newly infected",
